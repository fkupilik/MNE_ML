...
def read_data(param):
    """
    Load the data of all 250 subjects in the BrainVision format
    :param param: configuration object
    :return: epoched EEG data and labels
    """
    epochs_target = []
    epochs_non_target = []
    try:
        ...
        for folder_name in dirs:
            ...
            if len(file_name_vhdr) == 0:
                continue
            raw = mne.io.read_raw_brainvision(file_name_vhdr[0], preload=True)  # read raw data of the subject
            raw.filter(l_freq=param.l_freq, h_freq=param.h_freq)  # band-pass filtering
            if (raw.info['nchan'] == 4):
            ...
            line = text[2]
            event_id_target = int(line.split(": ")[1])  # get the thought number
            ...
            epochs_t_subject = mne.Epochs(raw, events=events_loaded[0], event_id=event_id_target, tmin=param.t_min,
                                          tmax=param.t_max, baseline=param.baseline)  # target epochs
            ...
            epochs_n_subject = mne.Epochs(raw, events=events_loaded[0], event_id=non_target_random, tmin=param.t_min,
                                          tmax=param.t_max, baseline=param.baseline)  # non-target epochs
    ...
    #   artifact removal by the peak-to-peak amplitude greater than 150 μV
    ...
    scaler = mne.decoding.Scaler(epochs_all.info, scalings=scalings)    # V -> μV

    X = epochs_all.get_data()
    X = scaler.fit_transform(X)

    # generate labels for target and non-target epochs
